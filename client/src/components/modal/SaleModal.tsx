...
const SaleModal = () => {
  ...
  if (!data || !data._id) {
    toastMessage({ icon: 'error', text: 'Product data is missing!' });
    return;
  }

  // Convert quantity to a number and validate it
  const quantityNum = Number(updateDate.quantity);
  if (isNaN(quantityNum) || quantityNum <= 0) {
    toastMessage({ icon: 'error', text: 'Invalid quantity!' });
    return;
  }

  // Ensure stock is available
  if (quantityNum > (data?.stock || 0)) {
    toastMessage({ icon: 'error', text: `Only ${data?.stock} items available in stock!` });
    return;
  }
    const payload = {
      ...updateDate,
